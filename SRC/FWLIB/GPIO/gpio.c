/**
  ...
  * @brief   超核K60固件库 GPIO 函数库 API函数
	修改说明：
	    对初始化函数进行了修改，减少对NVIC函数的使用步骤即可使用中断
  ******************************************************************************
  */
 #include "gpio.h"
...
void GPIO_Init(GPIO_InitTypeDef* GPIO_InitStruct)
{
	...
	if((GPIO_InitStruct->GPIO_IRQMode) >= 0x08)
	{
     switch((uint32_t)GPIOx)
		  {
				 case PTA_BASE:NVIC_EnableIRQ(PORTA_IRQn);break; //开启PORTA口使能时钟，在设置前首先开启使能时钟参见k10手册268页，
	       case PTB_BASE:NVIC_EnableIRQ(PORTB_IRQn);break;	//开启PORTB口使能时钟
	       case PTC_BASE:NVIC_EnableIRQ(PORTC_IRQn);break;	//开启PORTC口使能时钟
	       case PTD_BASE:NVIC_EnableIRQ(PORTD_IRQn);break;	//开启PORTD口使能时钟
	       case PTE_BASE:NVIC_EnableIRQ(PORTE_IRQn);break;	//开启PORTE口使能时钟
	       default : break;
      }
  }
}
