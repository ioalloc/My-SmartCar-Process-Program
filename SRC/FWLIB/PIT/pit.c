...
void PIT_ITConfig(uint8_t PITx, uint16_t PIT_IT, FunctionalState NewState)
{
	...
	if(PIT_IT == PIT_IT_TIF)
	{
		...
		if(ENABLE == NewState)
		{
			switch(PITx)
			{
			   case PIT0: NVIC_EnableIRQ(PIT0_IRQn);break;
			   case PIT1: NVIC_EnableIRQ(PIT1_IRQn);break;
			   case PIT2: NVIC_EnableIRQ(PIT2_IRQn);break;
				 case PIT3: NVIC_EnableIRQ(PIT3_IRQn);break;
				 default:break;
			}
    }
		else
       ...
       switch(PITx)
		   {
			   case PIT0: NVIC_DisableIRQ(PIT0_IRQn);break;
			   case PIT1: NVIC_DisableIRQ(PIT1_IRQn);break;
			   case PIT2: NVIC_DisableIRQ(PIT2_IRQn);break;
				 case PIT3: NVIC_DisableIRQ(PIT3_IRQn);break;
				 default:break;
			 }
    }
	}
